...
- ✨ **跨设备同步**：支持插件设置（白名单、黑名单、显示策略、语言偏好、开关状态）的跨设备同步，登录浏览器账号即可自动同步。
- 🛠️ **存储层重构**：引入统一的存储管理模块，采用 sync + local 双写策略，兼顾同步便捷性与极速访问性能。
- 🛠️ **无缝迁移**：老用户升级后会自动触发数据迁移逻辑，将既有本地数据无缝迁移至同步存储空间。
