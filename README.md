...
- [ ] **市场研究**：
  - [ ] https://xnapper.com/zh-CN/pricing
  - [ ] vscode 插件也有 codesnap
...
- [ ] **功能深挖**：专注于图片功能，将其做到最强：
  ...
  - [ ] 加强图片美化功能。场景是我截了一个图，这时候上传到网站的时候，网站对图片的宽高有要求。这时候我想能把我的图片做美化以后，导出符合要求的图片。让用户自己输入目标宽高，然后我压缩图片，并减少或者增大边距来符合比例？还是有更好的方案？帮我并对比可行方案

### v0.0.11 (2026-04-02)

- ✨ **名单管理**：新增白名单与黑名单手动管理功能，支持域名增删与冲突自动清理。
- ✨ **实时状态**：Popup 设置页实时展示当前站点的工具栏激活状态 (Active/Inactive)。
- 🛠️ **工程优化**：全面转向 `viteplus` (`vp`) 驱动的开发与构建体系，完善项目 `TODO` 路线图。

### v0.0.10 (2026-03-27)

- 💄 **样式优化**：采用卡片式设计重构功能开关，消除无用留白与避免滚动条出现。
- 🛠️ **界面重构**：引入标签页(Tab)机制，分离核心功能开关与高级设置模块。
- 🐛 **交互修复**：移除禁用状态逻辑联动，允许独立配置不同功能设定。

### v0.0.9 (2026-03-27)

- ✨ **显示策略**：新增四种全局工具栏显示策略（默认拦截/默认放行等）。
- ✨ **站点名单**：支持特定站点的白名单与黑名单配置。
- 🛠️ **快捷操作**：支持在插件 Popup 及右键菜单中快速添加或移除当前网站。

### v0.0.8 (2026-03-23)

- ✨ **图片导航**：预览模式支持方向键切换图片（←/↑ 上一张，→/↓ 下一张），并提供左右箭头按钮和图片计数器。
- 🛠️ **边界提示**：切换到第一张或最后一张时显示 toast 提示（"已经是第一张了"/"已经是最后一张了"）。
- 🌐 **国际化**：同步更新图片导航相关的中英文翻译。
